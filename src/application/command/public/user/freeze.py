...
@dataclass
class UserFreezingCommand:
    initiator_id: UUID
    user_id: UUID


class UserFreezingUseCase(BaseUseCase):
    async def execute(self, command: UserFreezingCommand) -> UserSimpleDTO:
        action = "заморозка пользователя"
